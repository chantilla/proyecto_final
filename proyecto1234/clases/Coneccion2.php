<?php
    $dsn = 'mysql:dbname=elecciones;host=localhost';
    $usuario = 'root';
    $clave = '';   
        try {
    ...
    function consultaA($coneccion, $sql)
    {
      ...
      try {
          ...
          {
              case "INSERT":
                  $msgerror = "Losentimos el numero de Dui ingresado es invalido";
  ...
  if ($objeto != NULL) {
    if (is_array($objeto)) {
      ...
      foreach ($objeto as $datos) {
        ...
        if($editar != "0" ){
          $tabla .=  "<td><a href=".$form."alumnoBuscar.php?id=".$datos['id_partido'].">Modificar</a></td>";
          $tabla .=  "<td><a href=".$form."alumnoBorrar.php?id=".$datos['id_partido'].">Eliminar</a></td>";  
